...
from functions.dyna import getdynaproblems
from functions.switch import getswitchstatus
from shuttlelib.utils.logger import logger
from functions.clientunique import client

async def do_get_problems():
    #Integration test with DYNA
    timedyna = "now-30m"
    switchednamespaces = await getswitchstatus(functional_environment="pro")
    print("## Dynatrace Open Problems")

    problems = await getdynaproblems(timedyna=timedyna, switchednamespaces=switchednamespaces)
    print("## Open Problems")
    print(problems)        
    
    cluster_problem = problems['cluster']
    region_problem = problems['region']
    namespace_problem = problems['namespace']
    microservice_problem = problems['microservice']
    
    return cluster_problem, region_problem, namespace_problem, microservice_problem
